...
# docker buildx build --tag oracle --file .pi/Dockerfile --platform linux/arm64 --cache-from oracle:cache --cache-to oracle:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar oracle
...
sudo bash -c "echo oracle > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/oracle.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/oracle.img
